 ...
 *
 * `/* /index.html 200` matches an extensionless path before anything else is
 * tried, so without these `/projects/dunx` served the landing page's metadata.
 */
const redirects = [
  '# Generated by scripts/shells.ts. Edit that, not this.',
  '#',
  '# One rule per route with its own metadata, because the catch-all below',
  '# matches an extensionless path before anything else is tried.',
  ...all.map((shell) => `/${shell.path}    /${shell.path}.html    200`),
  '',
